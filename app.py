...
def predict():
    ...
    if form.validate_on_submit():
        ...
        # Redirect to the results page with the prediction result as a URL parameter
        return redirect(url_for('result', prediction=f"{prediction:,.0f}"))
    return render_template("predict.html", title="Predict", form=form)

@app.route("/result")
def result():
    # Retrieve the prediction from the URL parameters
